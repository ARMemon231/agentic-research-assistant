...
agentic_researcher/
├── backend/
│   ├── main.py              ← FastAPI app (port 8000) — SSE + history endpoints
│   ├── requirements.txt
...
| MistralAI API key | Powers the LLM for validation, planning, and report writing |
...
**Get an MistralAI API key:** https://platform.MistralAI.com/api-keys  
...
Create a file called `.env` inside `agentic_researcher/backend/`:
...
> **Minimum to get started:** You only need `MistralAI_API_KEY` and `TAVILY_API_KEY`.  
...
INFO     | Research agent ready. Server is up at http://localhost:8000
...
```bash
cd agentic_researcher/frontend
```
...
**Backend won't start — `MistralAI_API_KEY` required**
...
→ The backend isn't running. Start it first: `uvicorn main:app --reload --port 8000`
...
|----------|---------|-----|
| Terminal 1 (Backend) | `cd backend && uvicorn main:app --reload --port 8000` | http://localhost:8000/docs |
