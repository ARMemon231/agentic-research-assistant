...
NEW CONCEPT: Pydantic Settings
  In Lecture 18/19 we used os.getenv("MistralAI_API_KEY") scattered throughout the code.
  ...
  Instead of os.getenv("MistralAI_API_KEY") everywhere, we use:
    from app.config import settings
    key = settings.MistralAI_api_key   ← type-checked, validated, single source of truth

  This follows the same pattern as Lecture 18's config.py.

USAGE:
  from app.config import settings
  print(settings.MistralAI_api_key)
...
class Settings(BaseSettings):
    ...
    Fields without a default value (e.g. MistralAI_api_key) are REQUIRED.
    ...
    # mistral-small-latest is the recommended model: fast, affordable, capable enough
    # for research tasks. Students can change this to "mistrsl-large-latest" for better quality.
